...
#
# workflow-service     - this is the actual production image that is much
...
ARG AIRFLOW_CONSTRAINTS_LOCATION="/tmp/constraints.txt"
...
##############################################################################################
FROM ${PYTHON_BASE_IMAGE} as workflow-service
SHELL ["/bin/bash", "-o", "pipefail", "-e", "-u", "-x", "-c"]
...
ARG AIRFLOW_USER_HOME_DIR=/opt/airflow
...
RUN chmod a+x /clean-logs && \
    ...
    chmod -R 777 ${AIRFLOW_HOME}/scripts/*
